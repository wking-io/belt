...
it.effect("sanitizes stored values against current config without deleting removed fieldset snapshots", () =>
  withProject(function*(cwd) {
    const config = defineControlPanel({
      fieldsets: {
        scene: {
    ...
    (cwd) =>
      Effect.provide(
        Effect.gen(function*() {
          ...
          yield* Effect.promise(() => mkdir(path.join(cwd, ".toolbar", "control-panel"), { recursive: true }));
