    ...
    withTempDir((cwd) =>
      Effect.gen(function* () {
        ...
        const message = yield* Effect.catchTag(
          ...
          "MissingToolbarConfigError",
          (error) =>
            Effect.succeed(error instanceof MissingToolbarConfigError ? error.cwd : "wrong"),
        );
...
it.effect(
  "fails with InvalidToolbarConfigExportError when the default export is not a toolbar config",
  () =>
    withLive(
      withTempDir((cwd) =>
        Effect.gen(function* () {
          ...
          const tag = yield* Effect.catchTag(
            ...
            (error) =>
              Effect.succeed(
                error instanceof InvalidToolbarConfigExportError ? error._tag : "wrong",
              ),
          );
...
it.effect(
  "fails with InvalidToolbarConfigExportError when a Toolbar Definition contains invalid config",
  () =>
    withLive(
      withTempDir((cwd) =>
        Effect.gen(function* () {
          yield* writeTempFile(
            cwd,
            "toolbar.config.mjs",
            "export default { toolbarConfig: { tools: [{ tool: { id: 'bad' } }] } };\n",
          );
          const configService = yield* ToolbarConfigService;
    ...
    withTempDir((cwd) =>
      Effect.gen(function* () {
        ...
        const tag = yield* Effect.catchTag(
          ...
          (error) =>
            Effect.succeed(
              error instanceof UnsupportedToolbarConfigFormatError ? error._tag : "wrong",
            ),
        );
    ...
    withTempDir((cwd) =>
      Effect.gen(function* () {
        ...
        const tag = yield* Effect.catchTag(
          ...
          "ToolbarConfigModuleLoadError",
          (error) =>
            Effect.succeed(error instanceof ToolbarConfigModuleLoadError ? error._tag : "wrong"),
        );
...
function withLive<A, E, R>(
  effect: Effect.Effect<A, E, R | ToolbarConfigService>,
): Effect.Effect<A, E, R> {
  return Effect.provide(
    effect,
    Layer.provide(ToolbarConfigService.layer, Layer.mergeAll(NodeFileSystem.layer, NodePath.layer)),
  );
}
